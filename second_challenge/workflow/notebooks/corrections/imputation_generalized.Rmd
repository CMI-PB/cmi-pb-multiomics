...
assays <- c("ab_titer", "live_cell_percentages", "olink", "rnaseq")
...
seq_u <- union(rownames(datasets[["rnaseq"]]), rownames(datasets[["olink"]]))
seq_u_c <- union(seq_u, rownames(datasets[["live_cell_percentages"]]))
samples_remove <- setdiff(rownames(datasets[["ab_titer"]]), seq_u_c)

# filter abtiters
abtiters.2020_filt <- subset(datasets[["ab_titer"]], !(rownames(datasets[["ab_titer"]]) %in% samples_remove))

# filter olink and cytof
samples_remove <- setdiff(union(rownames(datasets[["olink"]]),
                                rownames(datasets[["live_cell_percentages"]])),
                                rownames(datasets[["rnaseq"]]))
olink.2020_filt <- subset(datasets[["olink"]], !(rownames(datasets[["olink"]]) %in% samples_remove))
cytof.2020_filt <- subset(datasets[["live_cell_percentages"]], !(rownames(datasets[["live_cell_percentages"]]) %in% samples_remove))
abtiters.2020_filt <- subset(abtiters.2020_filt, !(rownames(abtiters.2020_filt) %in% samples_remove))
...
# create input for merging rnaseq
rnaseq_merge = log2(datasets[["rnaseq"]] + 1)
...
# rnaseq
rnaseq_cols = colnames(datasets[["rnaseq"]])
rnaseq_imputed <- X_mat_complete_df[rnaseq_cols]

# cytof
cytof_cols = colnames(datasets[["live_cell_percentages"]])
...
# abtiters
abtiters_cols = colnames(datasets[["ab_titer"]])
...
rnaseq_filename = sprintf(template_filename, "rnaseq", year)
write_tsv(rnaseq_imputed, rnaseq_filename)

cytof_filename = sprintf(template_filename, "live_cell_percentages", year)
...
abtiters_filename = sprintf(template_filename, "ab_titer", year)
